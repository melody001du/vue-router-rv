 ...
 */
// 将之前分割后的每一个片段进行打分，生成最终的由path转变成的整个路径的正则表达式，后续会根据打分进行排序
// 比如：/d0/d01?p0=jeff 打分为[[80],[80]],生成的正则为：/^\/d0\/d01\?p0=jeff\/?$/i
export function tokensToParser(
  ...
  function parse(path: string): PathParams | null {
     ...
     * i从1开始，获取非静态路径的捕获结果
     * let re=/^\/d1\/([^/]+?)\/?$/i
     * '/d1/13'.match(re)  返回['/d1/13', '13', index: 0, input: '/d1/13', groups: undefined ]
     */
