  ...
  return () =>
    new Promise((resolve, reject) => {
      const next: NavigationGuardNext = (
        ...
        // 拒绝导航，即return false, 产生一个 NavigationFailure
        if (valid === false) {
          reject(
        ...
        } else if (isRouteLocation(valid)) { // 重定向到新的地址也会产生一个 NavigationFailure
          reject(
      ...
      // 统一将返回结果变成Promise，是得后续添加next调用
      let guardCall = Promise.resolve(guardReturn)
      // 守卫方法参数小于3个，即守卫内不调用next时，在.then中自动调用next
...
export function extractComponentsGuards(
  ...
  for (const record of matched) {
    ...
    for (const name in record.components) {
      ...
      // 路由组件还没挂载时跳过路由 update 和 leave守卫
