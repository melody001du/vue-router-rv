...
export function createRouter(options: RouterOptions): Router {
  ...
  function resolve(
    ...
    // currentRoute.value为 START_LOCATION_NORMALIZED
    ...
    if (__DEV__) {
      if (href.startsWith('//')) {
        ...
        warn(
          `No match found for location with path "${'path' in rawLocation ? rawLocation.path : rawLocation
          }"`
        )
      }
    }
  ...
  function push(to: RouteLocationRaw) {
    // 触发路由守卫和调用routerHistory.push|routerHistory.replace更改路由
  ...
  function navigate(
    ...
    // 抽取离开路由记录中组件的路由守卫 beforeRouteLeave
    // 会处理当前路由以及上层链路所有路由记录
    ...
    // 对于setup中使用的路由守卫，会被收集进 leaveGuards 中
    ...
    return (
      // guards 是被包装后的方法，执行返回守卫方法返回结果的Promise
      // runGuardQueue会捕捉第一个reject状态的Promise
      runGuardQueue(guards)
        .then(() => {
          // 如果 beforeRouteLeave 全部成功执行
          // 执行 beforeEach 全局守卫
          guards = []
          // 这里的守卫是通过 routet.beforeEach 进行添加的
        ...
        .then(() => {
          // 如果 beforeEach 全部成功执行
          // 执行 beforeRouteUpdate 守卫
          // 抽取组件中的 beforeRouteUpdate
          // 从这里可以看出：beforeRouteUpdate 在导航之前已经导航过的路由记录时触发
          guards = extractComponentsGuards(
            updatingRecords,
            'beforeRouteUpdate',
            to,
          ...
          // 在setup中使用的路由守卫会被放进 updateGuards 中
